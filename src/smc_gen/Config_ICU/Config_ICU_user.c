...
/* Switches
#define SW1             (R_PORT1->PCNTR2_b.PIDR08)
#define SW1_PDR         (R_PORT1->PCNTR1_b.PDR08) // configurar el pin 8 de PORT1 como entrada
#define SW1_PMR         (R_PORT1->PCNTR1_b.PMR08) // configurar el pin en modo normal
*/
#include "Pin.h"
/* End user code. Do not edit comment generated here */
...
void r_Config_ICU_irq4_interrupt(void)
{
	...
	if (PIN_READ(S1_BOTON) == 0){
		boton = 1;
	}
	else if (PIN_READ(S1_BOTON) == 1){
		boton = 0;
	}
    /* End user code. Do not edit comment generated here */
